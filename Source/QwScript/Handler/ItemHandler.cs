...
{
    internal class ItemHandler : BaseHandler, INewIndexer
    {
        private object _instance;
        private IMethod _method;
        private bool _getter;

        public ItemHandler(ScriptState luaState)
            : base(luaState)
        {
        }
        ...
        public override int Reg2Env()
        {
            ...
            if (_getter)
            {
                var key = DataHelper.GetObject(luaState, -1);
                var val = _method.Invoke(Instance, key);
        ...
        public void SetValue(object value)
        {
            if (!_getter)
            {
                var key = DataHelper.GetObject(State, -2);
                _method.Invoke(Instance, key, value);
            }
        }
    }
}
